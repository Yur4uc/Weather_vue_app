...
import axios from 'axios'
  ...
  export default {
    ...
    methods: {
      getWeather() {
        if(this.city.trim().length < 2) { // trim обрізує пробіли до і після строки

          this.error = "Потрібно більше ніж один символ :)"
          return false
        ...
        axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${this.city}&units=metric&appid=3caabe6c26f960026733be78775dd614`)
...
<template>
  <div class="wrapper">
    ...
    <button disabled v-else>Please write your city</button>
